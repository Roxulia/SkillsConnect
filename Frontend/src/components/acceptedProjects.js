...
function AcceptedProjects({user})
{
    ...
    useEffect(()=>{
        ...
        const fetchProjects = async () => {
            try {
                const response = await axios.get("http://localhost:8080/api/user/accepted-projects", { withCredentials: true });
    ...
    return (<div className="p-3 my-3">
        <h2 className="text-xl font-bold text-sky-400 mb-4">Accepted Projects</h2>
            ...
            {projects.map((project) => (
                ...
                <div className="flex justify-between items-center text-sky-400">
                    ...
                    <button
                        ...
                        onClick={() => navigate("/ongoingproject", { state: project })}
                    >
                        Detail
                    </button>
                </div>
